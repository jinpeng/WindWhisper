...
- (void)initializeSubviews
{
    ...
    /// Initialize the WindWhisper logo label
    ...
    /// Initialize the WindWhisper title label
    ...
    self->_solTitleLabel.text = @"WindWhisper";
    ...
    for(SOLWeatherView *weatherView in self->_pagingScrollView.subviews) {
        if(weatherView.local == NO) {
            ...
            if([[NSDate date]timeIntervalSinceDate:weatherData.timestamp] >= kMIN_TIME_SINCE_UPDATE || !weatherView.hasData) {
                CZLog(@"SOLMainViewController", @"Updating Weather Data for %@ - %@, Time Since: %f", weatherData.placemark.locality, weatherData.placemark.administrativeArea, [[NSDate date]timeIntervalSinceDate:weatherData.timestamp]);
                ...
                [[SOLWundergroundDownloader sharedDownloader]dataForPlacemark:weatherData.placemark withTag:weatherView.tag completion:^(SOLWeatherData *data) {
                    if (data) {
                        // Success
                        CZLog(@"SOLMainViewController", @"Download finished for weather view with tag: %ld", (long)weatherView.tag);
                        ...
                        // Failure
                        CZLog(@"SOLMainViewController", @"Download failed for weather view with tag: %ld", (long)weatherView.tag);
                ...
                CZLog(@"SOLMainViewController", @"Not Updating Weather Data for %@ - %@, Time Since: %f", weatherData.placemark.locality, weatherData.placemark.administrativeArea, [[NSDate date]timeIntervalSinceDate:weatherData.timestamp]);
            }
        }
    }
}
...
- (void)updateWeatherView:(SOLWeatherView *)weatherView withData:(SOLWeatherData *)data
{
    ...
    CZLog(@"SOLMainViewController", @"Updating labels for weather view with tag: %ld", (long)weatherView.tag);
    ...
    if (city == nil)
        city = data.placemark.administrativeArea;
...
{
    CZLog(@"SOLMainViewController", @"Adding Weather View for Location %@, %@", placemark.locality, placemark.administrativeArea);
    
    /// Get cached weather data for the added placemark if it exists
    NSInteger tag;
    if(placemark.locality != nil) {
        tag = placemark.locality.hash;
    } else {
        tag = placemark.administrativeArea.hash;
    }
    SOLWeatherData *weatherData = [self->_weatherData objectForKey:[NSNumber numberWithInteger:placemark.administrativeArea.hash]];
...
{
    CZLog(@"SOLMainViewController", @"Moved Weather Tag at Index %ld to Index %ld", (long)sourceIndex, (long)destinationIndex);
...
{
    CZLog(@"SOLMainViewController", @"Removed Weather View with Tag: %ld", (long)tag);
...
{
    CZLog(@"SOLMainViewController", @"Download failed for weather view with tag: %ld", (long)tag);
...
{
    CZLog(@"SOLMainViewController", @"Download finished for weather view with tag: %ld", (long)tag);
